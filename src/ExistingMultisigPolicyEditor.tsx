...
export default function ExistingMultisigPolicyEditor({ account, network, networkParameters, accountSigningIntent = 'treasury', requestReturnTo, requestReturnLabel = 'Back to Treasury' }: { account: StellarAccountSnapshot; network: StellarNetwork; networkParameters: StellarNetworkParameters; accountSigningIntent?: AccountSigningIntent; requestReturnTo?: string; requestReturnLabel?: string }) {
  ...
  return (
    <div className="max-w-3xl">
      ...
      {step === 'signers' && <section className="rounded-2xl border border-black/10 bg-white p-5 dark:border-white/10 dark:bg-white/5 sm:p-6">
        ...
        <div className="mt-6 flex justify-end"><button type="button" disabled={!signerStepValid} onClick={() => setStep('approvals')} className="inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-5 py-3 text-sm font-semibold text-white disabled:opacity-40">Continue <ArrowRight className="h-4 w-4" /></button></div>
      </section>}

      {step === 'approvals' && <section className="rounded-2xl border border-black/10 bg-white p-5 dark:border-white/10 dark:bg-white/5 sm:p-6">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="rounded-xl bg-black/[0.035] p-4 dark:bg-white/[0.04]"><div className="text-sm font-semibold">Standard transactions</div><select value={paymentQuorum} onChange={(event) => changePayment(Number(event.target.value))} className="mt-3 w-full rounded-xl border border-black/10 bg-transparent px-3 py-3 text-lg font-bold dark:border-white/10">{Array.from({ length: participantCount }, (_, index) => index + 1).map((count) => <option key={count} value={count}>{count} of {participantCount}</option>)}</select></label><label className="rounded-xl bg-black/[0.035] p-4 dark:bg-white/[0.04]"><div className="text-sm font-semibold">Core account control</div><select value={adminQuorum} onChange={(event) => changeAdmin(Number(event.target.value))} className="mt-3 w-full rounded-xl border border-black/10 bg-transparent px-3 py-3 text-lg font-bold dark:border-white/10">{Array.from({ length: participantCount - paymentQuorum + 1 }, (_, index) => paymentQuorum + index).map((count) => <option key={count} value={count}>{count} of {participantCount}</option>)}</select></label></div>
        ...
        <div className="mt-6 flex justify-between gap-3"><button type="button" onClick={() => setStep('signers')} className="inline-flex items-center gap-2 rounded-xl border border-black/10 px-4 py-3 text-sm font-semibold dark:border-white/10"><ArrowLeft className="h-4 w-4" />Back</button><button type="button" disabled={!approvalStepValid} onClick={() => { setAcknowledged(false); setStep('review'); }} className="inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-5 py-3 text-sm font-semibold text-white disabled:opacity-40">Check changes <ArrowRight className="h-4 w-4" /></button></div>
      </section>}

      {step === 'review' && <section className="rounded-2xl border border-black/10 bg-white p-5 dark:border-white/10 dark:bg-white/5 sm:p-6">
        ...
        <div className="mt-6 flex justify-between gap-3"><button type="button" onClick={() => setStep('approvals')} className="inline-flex items-center gap-2 rounded-xl border border-black/10 px-4 py-3 text-sm font-semibold dark:border-white/10"><ArrowLeft className="h-4 w-4" />Back</button><button type="button" onClick={() => void continueToReview()} disabled={generating || !canCreate} className="inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-5 py-3 text-sm font-semibold text-white disabled:opacity-40">{generating && <LoaderCircle className="h-4 w-4 animate-spin" />}{generating ? 'Preparing review...' : <>Create change transaction <ArrowRight className="h-4 w-4" /></>}</button></div>
      </section>}
    </div>
  );
}
