              ...
              <div className="mt-6 space-y-5">
                ...
                {directSubmission ? (
                  <section className="rounded-2xl border border-emerald-500/25 bg-emerald-500/[0.07] p-5 sm:p-6">
                    <div className="flex gap-3"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-emerald-600" /><div className="min-w-0"><h2 className="text-xl font-bold">Transaction confirmed</h2><p className="mt-2 text-sm leading-6 text-neutral-600 dark:text-neutral-300">Stellar confirmed the exact reviewed transaction without requiring another wallet signature.</p><div className="mt-3 text-xs text-neutral-500 dark:text-neutral-400">Ledger {directSubmission.ledger.toLocaleString()}</div><div className="mt-1 break-all font-mono text-xs text-neutral-600 dark:text-neutral-300">{directSubmission.hash}</div><a href={transactionExplorerUrl(network, directSubmission.hash)} target="_blank" rel="noreferrer" className="mt-4 inline-flex text-sm font-semibold text-emerald-700 hover:underline dark:text-emerald-300">View on Stellar Expert</a></div></div>
                  </section>
                ) : directSubmitReady ? (
                  <section className="rounded-2xl border border-emerald-500/25 bg-emerald-500/[0.07] p-5 sm:p-6">
