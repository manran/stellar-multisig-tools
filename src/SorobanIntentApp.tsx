      ...
      <main className="px-4 py-7 sm:px-6 lg:px-8 lg:py-8">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mt-6 space-y-5">
            <section className="mst-intent-summary">
            ...
            {intent.privateContext?.initialPrivateNote && <section className="mst-intent-evidence-section"><div className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-400">Private Note</div><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-neutral-700 dark:text-neutral-200">{intent.privateContext.initialPrivateNote.text}</p></section>}

            {invocationFacts.length > 0 && <section className="mst-intent-evidence-section"><h2 className="text-xl font-bold">Authorized invocation tree</h2><p className="mt-2 text-sm leading-6 text-neutral-600 dark:text-neutral-300">This tree shows the authorization-bearing calls discovered by Soroban. Simulation effects below are broader evidence and may include internal calls that do not require a separate AUTH entry.</p><div className="mt-4 space-y-3">{invocationFacts.map((fact, index) => <InvocationTree key={`${fact.label}-${index}`} node={fact} />)}</div></section>}

            <section className="mst-intent-evidence-section"><h2 className="text-xl font-bold">Simulation effects at authorization</h2><p className="mt-2 text-sm leading-6 text-neutral-600 dark:text-neutral-300">This is the recording-simulation evidence fixed into the AuthorizationPlan. Plugins may improve labels, but they do not change the comparison or safety thresholds.</p><div className="mt-4"><SimulationEffectsView effects={intent.authorizationPlan.effects} /></div></section>
