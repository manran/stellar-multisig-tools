# MultiSigTools Human UX Architecture
...
4. promotion must not silently change ledger semantics. If the first recipient is not an active account, remain in the single-recipient path because an XLM payment may become `CreateAccount` while multiple-recipient payment does not create accounts automatically;
