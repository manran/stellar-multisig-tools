...
Classic Service Payment/batch and Account Creation are now business-first Prepare capabilities. A caller names the scoped source account and either payment rows or an explicit inactive destination + starting XLM balance; MultiSigTools owns fresh sequence, fee, lifetime, XDR construction, inspection, and source-requirement derivation. `classic.payment.prepare` and `classic.account.create.prepare` are separate stateless operations because `Payment` and `CreateAccount` have different ledger preconditions. Human composition may switch between them without discarding compatible draft fields, but destination existence never silently changes the selected operation. Integration `POST /request` (relative to the public protocol base) may compose Payment directly from semantic input; CreateAccount currently proceeds through its Prepare result into the same exact-XDR Request lifecycle. Other Classic capabilities become semantic only when an existing stable composer is deliberately promoted; there is no generic transaction DSL.
...
Audience-specific business packaging is governed by `apps/internal-docs/content/stellar/product/audience-projection-model.md`: Human uses one current action, signer Agent uses typed `task`, and external Integration uses `job`. All three derive from the same Request/Intent facts rather than creating new workflow state.
