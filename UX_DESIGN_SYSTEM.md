...
| **4 Submit** | Authorization is sufficient; wait for execution conditions and explicitly submit | Proposal |
...
- Guided Contract Call Review automatically runs the required recording simulation. Raw/unprepared imported Soroban XDR keeps the recording RPC check explicit because it originated outside the guided composer. An imported prepared Soroban XDR must preserve its embedded pre-freeze authorization evidence and continue that authorization directly; do not record-simulate over it and silently erase another signer's contribution.
...
- Soroban pre-freeze G-account authorization uses the same XDR transport principle at the correct lifecycle boundary: partial auth-entry evidence may be copied or scanned to another current signer before Proposal freeze. The receiving signer imports that prepared XDR and continues existing authorization. Do not require one person to sequentially control every signer wallet. Once pre-freeze authorization is complete, a different transaction-envelope signer may receive the prepared XDR in the same way before the ordinary Proposal/Sign flow begins.
