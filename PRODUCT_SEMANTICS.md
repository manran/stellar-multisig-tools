...
Execution/audit language follows observed facts rather than workflow aspiration. A Soroban `execution_prepared` fact means MultiSigTools successfully late-bound the source, ran enforcing simulation, produced the exact transaction hash, and durably retained the preparation summary; it does **not** mean a browser copied the XDR, another system received it, or Stellar accepted it. For Classic submission, an exact transaction may be reconciled from Horizon after another party submitted it: that produces a system confirmation fact but must not attribute the current viewer as submitter. `submitted by <actor>` is retained only when MultiSigTools itself observed that actor's submission call succeed; `confirmed` requires the exact successful transaction hash and ledger, regardless of submitter provenance.
