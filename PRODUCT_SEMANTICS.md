...
Detached AUTH contributions are append-only and verified against the exact Soroban authorization preimage plus current live signer policy. `authorization_ready` is a complete coordination state and does not construct a transaction. Only then may an eligible Human/Agent choose `executionSource`; the service loads a fresh sequence, materializes the final transaction, runs enforcing simulation, and returns unsigned final XDR. If that final envelope requires multisig, it enters the ordinary Proposal/Sign lifecycle. A different executor may therefore sign the envelope than the actor(s) who supplied detached Soroban AUTH.
...
S3A/S3B/S3C contract-account work remains a separate contract/network-enforced credential path. A configured C-account adapter may stage contract-defined `ScVal` evidence only under its explicit provenance and must pass enforcing simulation; unknown C-accounts and delegated authorization remain inspect-only/fail-closed. A custom payload being present is not local proof of `__check_auth` validity. Simulation output is execution evidence, never a signature. See `SOROBAN_AUTHORIZATION.md`.
